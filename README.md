...
(`process.html?project=&part=`)。延续平台思路:Claude 只产出**结构化工艺路线**
...
数量/单价/金额/价格来源/置信度),并允许 Claude **联网检索**当前材料、外购件、加工费率等
行情作为依据(服务端 `web_search` 工具),把价格出处写进 price_references(可追溯)。平台做
...
> 视图图片 / 规格文本 / 已知报价),生成与重新生成时一并提交(multipart),Claude 优先采用 ——
> 让结果更贴合你的实际约束。
...
解析/校验/拆解(Claude 调用,数十秒)与几何/2D/3D 导入(CAD,数秒至数分钟)都改为
**异步任务**:接口立即返回 `{task_id}`,前端轮询进度,完成后取结果渲染。

- 进程内队列(标准库 `ThreadPoolExecutor`,**零外部中间件**,契合私有化/单机优先);
  CAD 任务经全局锁串行化(OCCT 非线程安全),Claude 任务可并发。
